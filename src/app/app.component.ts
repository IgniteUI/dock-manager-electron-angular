...
import { IgcContentPane, IgcDockManagerComponent, IgcDockManagerLayout, IgcDockManagerPaneType, IgcSplitPane, IgcPaneHeaderConnectionEventArgs, IgcSplitPaneOrientation } from 'igniteui-dockmanager';
//import './index.css';
...
export class AppComponent implements AfterViewInit {
    title = 'dockmanager-electron-angular';
    ...
    private currentWindow = window as IDockManagerWindow;
    private dm: IgcDockManagerComponent;
    ...
    public ngAfterViewInit() {
        this.dm = document.getElementById('dock') as IgcDockManagerComponent;

        this.dm.addEventListener('paneHeaderConnected', (event) => this.paneHeaderConnected(event));
        this.dm.addEventListener('paneHeaderDisconnected', (event) => this.paneHeaderDisconnected(event));
        this.dm.addEventListener('tabHeaderConnected', (event) => this.paneHeaderConnected(event));
        this.dm.addEventListener('tabHeaderDisconnected', (event) => this.paneHeaderDisconnected(event));
        this.dm.addEventListener('splitterResizeStart', () => this.disableContentPointerEvents(this.dm));
        this.dm.addEventListener('splitterResizeEnd', () => this.enableContentPointerEvents(this.dm));

        document.addEventListener('dragover', (event) => this.handleDocumentDragOver(event));
        document.addEventListener('drop', (event) => this.handleDocumentDrop(event));

        if (this.currentWindow.isMain !== false) {
            this.currentWindow.isMain = true;
        }
    ...
    */
    private paneHeaderConnected(event: CustomEvent<IgcPaneHeaderConnectionEventArgs>) {
    ...
    private paneHeaderDisconnected(event: CustomEvent<IgcPaneHeaderConnectionEventArgs>) {
        const element = event.detail.element;
        element.ondragstart = null;
        element.ondragend = null;
    }

    private async paneHeaderDragStart(pane: IgcContentPane, event: DragEvent) {
        event.dataTransfer.dropEffect = 'move';
        this.dm.draggedPane = pane;
    ...
    private paneDragStart(window: Window, pane: IgcContentPane) {
        const dockManager = window.document.getElementById('dock') as IgcDockManagerComponent;
    ...
    private async paneHeaderDragEnd(event: DragEvent) {
    ...
    private async paneDragEnd(window: Window) {
        const dockManager = window.document.getElementById('dock') as IgcDockManagerComponent;
        ...
        if (!this.currentWindow.isMain && (!dockManager.layout.rootPane.panes || !dockManager.layout.rootPane.panes.length)) {
    ...
    private handleDocumentDragOver(event: DragEvent) {
        event.preventDefault();
        event.dataTransfer.dropEffect = 'move';
        this.dm.dropPosition = {
            x: event.clientX,
            y: event.clientY
        };
    }

    private async handleDocumentDrop(event: DragEvent) {
        const contentId = (this.dm.draggedPane as IgcContentPane).contentId;

        const docked = await this.dm.dropPane();

        if (docked) {
            const contentElement = this.dm.querySelector('[slot=' + contentId + ']');
     ...
     * @returns 
     */
    private async droppedOutOfWindow(event: DragEvent) {
        const draggedPane = this.dm.draggedPane as IgcContentPane;

        // if there is a single pane in the window just move the window
        if (this.dm.layout.rootPane.panes.length === 1 && this.dm.layout.rootPane.panes[0] === draggedPane) {
            window.moveTo(event.screenX, event.screenY);
            return;
        }

        await this.dm.removePane(draggedPane);
        this.dm.layout = { ...this.dm.layout };

        draggedPane.isPinned = true;

        const contentElement = this.dm.querySelector('[slot=' + draggedPane.contentId + ']');
        ...
        this.openChildWindow(event.screenX, event.screenY, rect.width, rect.height, (childWindow: Window) => {
            ...
            const newDockManager = newDocument.getElementById('dock') as IgcDockManagerComponent;
    ...
    private async droppedInAnotherWindow() {
        const mainWindow = this.getMainWindow();
        const sourceDocument = mainWindow.dragStartWindow.document;
        const sourceDockManager = sourceDocument.getElementById('dock') as IgcDockManagerComponent;
        ...
        this.dm.appendChild(adoptedNode);
    }

    private enableContentPointerEvents(dockManager: IgcDockManagerComponent) {
     ...
     */
    private disableContentPointerEvents(dockManager: IgcDockManagerComponent) {
        for (const child of Array.from(dockManager.children)) {
            (child as HTMLElement).style.pointerEvents = 'none';
        }
    }

    private getMainWindow(): IMainDockManagerWindow {
        return this.currentWindow.isMain !== false ? this.currentWindow : this.currentWindow.mainWindow;
    }

    private openChildWindow(x: number, y: number, width: number, height: number, onOpen: (childWindow: Window) => void) {
        const mainWindow = this.getMainWindow();
    ...
    private executeForAllWindows(callback: (window: Window, param?: any) => void, param?: any) {
        const mainWindow = this.getMainWindow();
        callback.apply(this, [mainWindow, param]);

        if (mainWindow.childWindows) {
            mainWindow.childWindows.forEach(window => callback.apply(this, [window, param]));
        }
    }
}
